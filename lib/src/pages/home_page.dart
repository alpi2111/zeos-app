...
class _HomePageState extends State<HomePage> {
    ...
    return Scaffold(
        ...
        body: /*con.connectivityResult ?*/ ConnectivityWidget(
            ...
            builder: _prefs.isAdmin
                ...
                : (context, isOnline) =>
                    _crearHerramientasSucursal(isOnline) //no es el supervisor
            ) /*: Center(
  ...
  Widget _crearHerramientasSucursal(bool isOnline) {
    return FutureBuilder(
  ...
  _fabNoAdmin() {
    return FloatingActionButton(
      ...
      onPressed: () async {
        bool cerrar = await mostrarAlertaCerrarSesion(context, 'Cerrar Sesión', '¿Desea Cerrar Sesión?');
        if (cerrar) {
          _prefs.token = null;
  ...
  List<Widget> _pbtnNoAdmin() {
    return <Widget>[
      RaisedButton(
        onPressed: () {},
        child: Text('Agregar Herramienta', style: TextStyle(color: Colors.white),),
      ),
    ];
  }
}
